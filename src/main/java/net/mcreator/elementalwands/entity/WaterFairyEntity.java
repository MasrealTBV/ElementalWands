...
import net.mcreator.elementalwands.procedures.WaterFairyRightClickedOnEntityProcedure;
...
public class WaterFairyEntity extends Monster {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("frozen_ocean"), new ResourceLocation("warm_ocean"),
			...
			new ResourceLocation("deep_cold_ocean"), new ResourceLocation("lukewarm_ocean"), new ResourceLocation("beach"),
			new ResourceLocation("river"), new ResourceLocation("desert"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ElementalWandsModEntities.WATER_FAIRY.get(), 5, 1, 1));
	}

	public WaterFairyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ElementalWandsModEntities.WATER_FAIRY.get(), world);
	}

	public WaterFairyEntity(EntityType<WaterFairyEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = WaterFairyEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = WaterFairyEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = WaterFairyEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		WaterFairyRightClickedOnEntityProcedure.execute(world, entity);
		return retval;
	}
	...
	public static void init() {
		SpawnPlacements.register(ElementalWandsModEntities.WATER_FAIRY.get(), SpawnPlacements.Type.ON_GROUND,
