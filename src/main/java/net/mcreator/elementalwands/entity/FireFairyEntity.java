...
public class FireFairyEntity extends PathfinderMob {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ElementalWandsModEntities.FIRE_FAIRY.get(), 5, 1, 1));
	}

	public FireFairyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ElementalWandsModEntities.FIRE_FAIRY.get(), world);
	}

	public FireFairyEntity(EntityType<FireFairyEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = FireFairyEntity.this.getRandom();
				double dir_x = FireFairyEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = FireFairyEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = FireFairyEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(ElementalWandsModItems.FIRE_SHARD.get()));
	}
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		FireFairyRightClickedOnEntityProcedure.execute(world, entity);
		return retval;
	}
	...
	public static void init() {
		SpawnPlacements.register(ElementalWandsModEntities.FIRE_FAIRY.get(), SpawnPlacements.Type.ON_GROUND,
